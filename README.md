...
Rspamd is quite popular milter, which is super easy to use and have lot of advantages compared to older solutions like Spamassassin. The main advantage is that it can be extended with other modules to provide complex complex functionality while keeping only one single milter within Postfix configuration.

First install rspamd. We will use Rspamd's own repository to get lastest release and updates:
...
Now edit `/etc/postfix/main.cf` and add following lines:
